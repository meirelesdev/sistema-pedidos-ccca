...
export default class StockEntryRepositoryDatabase implements StockEntryRepository {
    ...
    async getByIdItem(idItem: number): Promise<StockEntry[]> {
        const stockEntrysData = await this.database.many("select * from ccca.stock_entry where id_item = $1", [idItem])
        const stockEntrys = []
        for(const stockEntryData of stockEntrysData) {
            stockEntrys.push(new StockEntry(stockEntryData.id_item, stockEntryData.operation, stockEntryData.quantity, new Date()))
        }
        return stockEntrys
    }
